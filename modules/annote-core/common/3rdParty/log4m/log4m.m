...
% =========================================================================
% ADDITIONAL HISTORY
%   2017/02/21  A.LEICHTMAN     Added specification of subclass when
%                               getting logger instance
%   2017/04/19  A.LEICHTMAN     Disabled file creation on instatiation
% =========================================================================
    ...
    methods (Static)
        function obj = getLogger( logPath, loggerClass)
            ...
            if(nargin == 0)
                logPath = 'log4m.log';
            elseif(nargin > 2)  % A.LEICHTMAN2017FEB21
                % error('getLogger only accepts one parameter input');
                error('getLogger only accepts one or two parameter input'); 
            end
            
            persistent localObj;
            if isempty(localObj) || ~isvalid(localObj)
               
                if exist('loggerClass', 'var') && exist(loggerClass', 'file')
                    localObj = eval(sprintf('%s(logPath)',loggerClass));
                else
...
%   Unless you're modifying this, these should be of little concern to you.
    ...
    methods (Access = protected)  % A.LEICHTMAN2017FEB21
...
%% WriteToFile    
        ...
        function writeLog(self,level,scriptName,message)
            ...
            if( self.commandWindowLevel <= level )
                % A.LEICHTMAN2017FEB21
                %fprintf('%s:%s\n', scriptName, message);
                self.writeCommandWindow(level,scriptName,message)
            end
